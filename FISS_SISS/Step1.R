...
for (i in 1:40) {
    ...
    FISSitems[,i+3] <- ifelse(FISSitems[,i+3]!=FISScr[i,],
                              ifelse(FISSitems[,i+3]==incr[1]|incr[2]|incr[3]|incr[4],0,NA)
                              ,1)
}



#FISSitems[FISSitems$cntry==60,]

#head(FISSitems)
#temp <- FISSitems[FISSitems$cntry==60, ]
#head(temp)

#for (i in 1:40) {
#  incr <- c(1,2,3,4,5)[!(c(1,2,3,4,5)%in%FISScr[i,])]
#  temp[,i+3] <- ifelse(temp[,i+3]!=FISScr[i,],
#                            ifelse(temp[,i+3]==incr[1]|incr[2]|incr[3]|incr[4],0,NA)
#                            ,1)
#}
#head(temp)
...
isr <- FISSitems[FISSitems$cntry==60,]
FISScoding <- as.data.frame(FISScoding)
for (i in 1:40) {
  FISScoding[,i+3] <- as.numeric(FISScoding[,i+3])
}

table(FISSitems$e1sb20,useNA = "always")
table(FISScoding$DE1SB20,useNA = "always")
all.equal(FISSitems[,4:43],FISScoding[4:43]) #inconsistencies in numbers of NA

FISSitemsNIS <- FISSitems[FISSitems$cntry!=60,]
FISScodingNIS <- FISScoding[FISScoding$cntry!=60,]
all.equal(FISSitemsNIS[,4:43],FISScodingNIS[4:43]) #inconsistencies in numbers of NA
dim(FISSitemsNIS)
dim(FISScodingNIS)

writexl::write_xlsx(x = FISSlist, file = "excel_file.xlsx")
table(FISSitemsNIS$e1sb20,useNA = "always")
table(FISScodingNIS[,43],useNA = "always")

FISSitemsNIS[FISSitemsNIS$cntry==60,]
...
# xlsx::write.xlsx(SISSlist$NAbyItem, file="NAbyItem.xlsx", sheetName="NAbyItem", row.names=FALSE)
# xlsx::write.xlsx(SISSlist$NAbyStud, file="NAbyStud.xlsx", sheetName="NAbyStud", row.names=FALSE)
# xlsx::write.xlsx(SISSlist$NAbycnt, file="NAbycnt.xlsx", sheetName="NAbycnt", row.names=FALSE)
# 
# 
# xlsx::write.xlsx(SISSlist$CRbyItem, file="CRbyItem.xlsx", sheetName="CRbyItem", row.names=FALSE)
# xlsx::write.xlsx(SISSlist$CRbyStud, file="CRbyStud.xlsx", sheetName="CRbyStud", row.names=FALSE)
# xlsx::write.xlsx(SISSlist$CRbycnt, file="CRbycnt.xlsx", sheetName="CRbycnt", row.names=FALSE)


#saveRDS(FISSitems,file = "FISSitems.Rds")
